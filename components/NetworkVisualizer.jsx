...
const NetworkNode = ({ position, type, status, id, onClick, throughput, isTraining, trainingProgress }) => {
    ...
    useFrame((state) => {
        ...
        if (meshRef.current) {
            ...
            // Pulsing effect for training nodes
            const pulseScale = isTraining ? 1 + Math.sin(state.clock.elapsedTime * 3) * 0.1 : 1;
            
            if (hovered) {
                meshRef.current.scale.lerp(new THREE.Vector3(1.3 * pulseScale, 1.3 * pulseScale, 1.3 * pulseScale), 0.1);
            } else {
    ...
    const color = type === 'gNB'
        ? (status === 'active' ? (isTraining ? '#10b981' : '#00f3ff') : '#f59e0b')
        : (isTraining ? '#f59e0b' : '#8b5cf6');
    ...
    return (
        <group ref={groupRef} position={[0, 0, 0]}>
            <mesh
                ref={meshRef}
                onClick={() => onClick({ id, type, status, isTraining, trainingProgress })}
            ...
            >
                ...
                <meshStandardMaterial
                    color={color}
                    emissive={color}
                    emissiveIntensity={hovered ? 0.8 : (isTraining ? 0.7 : 0.5)}
                    metalness={0.8}
                    roughness={0.2}
                />
            </mesh>
            ...
            {/* Glow effect */}
            <mesh scale={hovered ? 1.5 : (isTraining ? 1.4 : 1.2)}>
                <sphereGeometry args={[size, 32, 32]} />
                <meshBasicMaterial
                    color={color}
                    transparent
                    ...
                    <div className="node-tooltip">
                        ...
                        {isTraining && (
                            <div className="training-info">
                                <div>🧠 Training</div>
                                <div>Progress: {trainingProgress || 0}%</div>
                            </div>
                        )}
                    </div>
                </Html>
...
// Main 3D Scene
const NetworkScene = ({ topology, onNodeClick, viewMode, throughput, agents }) => {
    ...
    // Determine if nodes are training based on agent states
    const getNodeTrainingInfo = (nodeId) => {
        if (!agents) return { isTraining: false, trainingProgress: 0 };
        
        const agent = agents.find(a => a.id === nodeId || a.name.toLowerCase().includes(nodeId.toLowerCase()));
        return {
            isTraining: agent?.state === 'training',
            trainingProgress: agent?.trainingProgress || agent?.convergence || 0
        };
    };
    ...
    return (
        <>
            ...
            {topology.gNBs.map((gnb) => {
                const trainingInfo = getNodeTrainingInfo(gnb.id);
                return (
                    <NetworkNode
                        key={gnb.id}
                        id={gnb.id}
                        ...
                        trainingProgress={trainingInfo.trainingProgress}
                    />
                );
            })}

            {topology.UEs.map((ue) => {
                const trainingInfo = getNodeTrainingInfo(ue.id);
                return (
                    <NetworkNode
                        key={ue.id}
                        id={ue.id}

...
router.post('/system/autoconfig', (req, res) => {
    ...
    const emitUpdate = () => {
        const updateData = {
            ...
            topologyType: simulationState.currentTopologyType,
            metrics: generators.generateNetworkMetrics(),
            agents: generators.generateAgentStatesWithTopology(),
...
// Select topology manually
router.post('/network/topology/select', (req, res) => {
    const { type } = req.body;
    if (type) {
        simulationState.currentTopologyType = type;
        console.log(`🌐 [Network] Topology manually switched to ${type}`);

        // Reset training agents as the Twin environment has physically changed
        generators.resetTrainingStates();

        const io = req.app.get('io');
        const updateData = {
            topology: generators.generateNetworkTopology(type),
            topologyType: type,
            metrics: generators.generateNetworkMetrics(),
            timestamp: new Date().toISOString()
        };
        ...
        res.status(400).json({ success: false, message: 'Invalid topology type' });
    }
});

...
const getTopologyLayout = (type) => {
  ...
  switch (type) {
    ...
    case 'Mesh':
    default:
      // Grid/Mesh layout with 12 gNBs
      for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 3; j++) {
          const id = `gnb-${i * 3 + j + 1}`;
          gNBs.push({ id, type: 'gNB', x: 150 + i * 180, y: 100 + j * 120, status: 'active' });
          for (let k = 0; k < randomInt(2, 5); k++) {
            UEs.push({ id: `ue-${id}-${k}`, type: 'UE', x: 150 + i * 180 + randomRange(-40, 40), y: 100 + j * 120 + randomRange(-40, 40), connectedTo: id, status: 'active' });
          }
        }
      }
      return { gNBs, UEs };
...
export const generateNetworkTopology = (type = null) => {
  // Use auto-switching if no type provided and simulation is active
  const topologyType = type || (simulationState.active ? switchTopologyAutomatically() : simulationState.currentTopologyType);
  
  // Use internal engine to build layout based on topology type
  const { gNBs, UEs } = getTopologyLayout(topologyType);

  // Persist into simulation state so it doesn't reshuffle every frame unnecessarily
  simulationState.topology = { gNBs, UEs };

  return { gNBs, UEs, topologyType };
};
...
// Reset RL Agent states to simulate re-training on new environments
export const resetTrainingStates = () => {
  simulationState.agents = simulationState.agents.map(agent => ({
    ...agent,
    episodes: 0,
    avgReward: '0.1200',
    convergence: '12.00',
    state: 'training' // Force them back into training mode
  }));
};
...
// Automatic topology switching for training
export const switchTopologyAutomatically = () => {
  if (!simulationState.autoTopologySwitching || !simulationState.active) {
    return simulationState.currentTopologyType;
  }

  const now = Date.now();
  if (simulationState.lastTopologySwitch && 
      now - simulationState.lastTopologySwitch < simulationState.topologySwitchInterval) {
    return simulationState.currentTopologyType;
  }

  // Get next topology in the list
  const currentIndex = simulationState.availableTopologies.indexOf(simulationState.currentTopologyType);
  const nextIndex = (currentIndex + 1) % simulationState.availableTopologies.length;
  const nextTopology = simulationState.availableTopologies[nextIndex];

  simulationState.currentTopologyType = nextTopology;
  simulationState.lastTopologySwitch = now;

  // Reset training states when topology changes
  resetTrainingStates();

  console.log(`🔄 [Auto-Switch] Topology changed to ${nextTopology} for training`);
  return nextTopology;
};

// Enhanced agent states with topology-specific training
export const generateAgentStatesWithTopology = () => {
  if (!simulationState.active) return simulationState.agents;

  // Auto-switch topology if enabled
  switchTopologyAutomatically();

  // Update agents with current topology context
  simulationState.agents = simulationState.agents.map(agent => ({
    ...agent,
    episodes: agent.state === 'training' ? agent.episodes + 1 : agent.episodes,
    avgReward: agent.state === 'training'
    ...
    convergence: agent.state === 'training'
      ? Math.min(100, parseFloat(agent.convergence) + 0.01).toFixed(2)
      : agent.convergence,
    currentTopology: simulationState.currentTopologyType,
    trainingProgress: agent.state === 'training' 
      ? Math.min(100, parseFloat(agent.convergence) + randomRange(0, 2)).toFixed(2)
      : agent.convergence
  }));
